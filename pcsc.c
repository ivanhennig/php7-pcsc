...
// PHP5 reverse compatibility macros
#define Z_RESVAL_P(z) Z_RES_P(z)->handle
#define ZEND_REGISTER_RESOURCE(x,y,z) ZVAL_RES(x, zend_register_resource(y, z));
#define ZEND_FETCH_RESOURCE(x, y, z, t ,a, b) (x) = (y) zend_fetch_resource(Z_RES_P(z), a, b)

#undef RETVAL_STRING
#define RETVAL_STRING(x, y) do { if (y) { ZVAL_STRING(return_value, x); return; } else { ZVAL_STRING(return_value, x); efree ((char *)x); return; } } while (0)

#undef RETURN_STRING
#define RETURN_STRING(x, y) RETVAL_STRING(x, y);
...
PHP_FUNCTION(scard_connect)
{
  ...
  if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "rs", &ctx_res, &strReaderName, &strReaderNameLen) == FAILURE) {
    RETURN_NULL();
  }
